        ...
        extend: {
            filter: {
                ...
                register: function (event, callback) {
                    ...
                    // 如果是第一次加载插件，执行测试
                    if (event === 'after_render:html' && this._filters['after_render:html'].length === 1) {
                        this.runTest();
                    }
                },
                runTest: function () {
...
结束
          ...
          `;
                    ...
                    console.log('原始 mermaid 代码块数量:', (processedData.content.match(/```mermaid/g) || []).length);
                    console.log('转换后的容器数量:', (result.match(/mermaid-diagram-container/g) || []).length);
                }
            }
        }
    };
